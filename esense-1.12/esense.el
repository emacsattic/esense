...
(defconst esense-arity-syntax-table
  (let ((table (copy-syntax-table erlang-mode-syntax-table)))
    (modify-syntax-entry ?< "(" table)
    (modify-syntax-entry ?> ")" table)
    table)
  "Syntax table used when determining the arity of a function.
It treats < and > like parenthesis, so that parsing binaries
in the argument list is more robust.")

(defconst esense-newline-syntax-table
  (let ((table (copy-syntax-table erlang-mode-syntax-table)))
    (modify-syntax-entry 10 " " table)
    table)
  "Modified Erlang syntax table where newline has whitespace
syntax. Shouldn't this be the default?")
...
(defvar esense-minor-mode-map
  (let ((map (make-sparse-keymap)))
     (define-key map (kbd "<f1>") 'esense-do-something-at-point)
     (define-key map (kbd "C-<f1>") 'esense-go-to-documentation)
     (define-key map (kbd "M-<f1>") 'esense-go-to-function-documentation)
     (define-key map (kbd "C-<f11>") 'esense-distel-previous-error)
     (define-key map (kbd "C-<f12>") 'esense-distel-next-error)
...
(easy-mmode-define-minor-mode
  ...
  (if (esense-distel-available-p)
      (if esense-check-syntax-when-idle
          (unless esense-syntax-check-timer
            (setq esense-syntax-check-timer
                  (run-with-idle-timer
                   esense-check-syntax-when-idle t
                   'esense-check-current-buffer-syntax))

            (add-hook 'after-save-hook 'esense-check-current-buffer-syntax)))))
      ...
      (let* ((bound (save-excursion 
        ...
        (if (> (length symbol) 0)
            (if (and parent-symbol
                     ...
                     (save-excursion
                       (goto-char symbol-beginning-position)
                       (with-syntax-table esense-newline-syntax-table
...
(defun esense-start-completion (symbol-beginning-position 
                                completion-list
                                &optional inserter documentation-getter)
...
`source' indicating an actual source file."
  ...
  (let ((result (make-esense-erlang))
    ...
    (if (or (eq type 'source)
        ...
        (if (esense-distel-available-p)
            (esense-distel-parse-file result)
